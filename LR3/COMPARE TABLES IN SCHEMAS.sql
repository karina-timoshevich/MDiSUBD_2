...
CREATE OR REPLACE PROCEDURE compare_schemas (
    ...
    PROCEDURE topological_sort IS
        ...
        -- массив для контроля уже добавленных таблиц
    ...
    BEGIN
        FOR i IN 1..n LOOP
            v_sort(i).object_type := v_sorted_tables(i).object_type;
            ...
            IF v_sorted_tables(i).object_type = 'TABLE' THEN
                ...
                END LOOP;
            ELSE
                v_sort(i).dep_count := NULL;
            END IF;
        END LOOP;
        FOR i IN 1..n-1 LOOP
            FOR j IN i+1..n LOOP
                IF v_sort(i).object_type = 'TABLE' AND v_sort(j).object_type = 'TABLE' THEN
        ...
        FOR i IN 1..n LOOP
            v_sorted_tables.EXTEND;
            v_sorted_tables(v_sorted_tables.LAST) := table_rec(v_sort(i).object_type, v_sort(i).object_name, v_sort(i).has_cycle);
        END LOOP;
    END sort_tables;
